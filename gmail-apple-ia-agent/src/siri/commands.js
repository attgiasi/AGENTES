...
export async function handleSiriCommand(command, { db, body = {} }) {
  ...
  if (command === 'limpar-newsletters' || command === 'arquivar-newsletters-antigas') {
    const result = await runAgent({ db, settings, manual: true, query: 'newer_than:30d (list: OR category:promotions OR unsubscribe)' });
  ...
  if (command === 'criar-lembretes') {
    const result = await runAgent({ db, settings, manual: true, query: 'newer_than:14d (prazo OR vencimento OR responder OR cobrança OR boleto)' });
  ...
  if (command === 'criar-eventos') {
    const result = await runAgent({ db, settings, manual: true, query: 'newer_than:30d (reunião OR evento OR call OR convite OR agenda)' });
  ...
  if (command === 'preparar-respostas' || command === 'responder-hoje') {
    const result = await runAgent({ db, settings, manual: true, query: 'newer_than:7d (responder OR retorno OR aguardo OR confirmar)' });
  ...
  if (command === 'resumir-importantes' || command === 'listar-urgentes') {
    const result = await runAgent({ db, settings, manual: true, query: 'newer_than:7d (urgente OR importante OR prazo)' });
