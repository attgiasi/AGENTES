...
export function buildActionPlan({ email, decision, settings, db }) {
  ...
  for (const item of requested) {
    ...
    if (settings.agent.dryRun) {
      planned.push({ ...enriched, status: 'dry-run' });
      continue;
    }

    if (evaluation.allowed) {
      planned.push({ ...enriched, status: 'ready' });
      continue;
    }
    ...
    if (evaluation.requiresConfirmation) {
      const approval = db?.createApproval({
        action: item.name,
        risk: evaluation.risk,
        payload: { emailId: email.id, decision, action: item }
      });
      approvals.push(approval);
      planned.push({ ...enriched, status: 'pending_confirmation', approvalId: approval?.id });
      continue;
    }
...
export function actionsFromDecision(email, decision, settings) {
  ...
  if (decision.acao_recomendada === 'arquivar') actions.push({ name: 'archiveEmail' });
