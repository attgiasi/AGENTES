...
app.post('/api/emergency-stop', async (req, res) => {
  const settings = await loadSettings(db);
  settings.agent.emergencyStop = true;
  ...
  db.log('warn', 'security', 'Parada de emergência acionada pelo painel.');
  res.json(await loadSettings(db));
});

app.post('/api/resume-after-emergency', async (req, res) => {
  const settings = await loadSettings(db);
...
app.post('/api/run', async (req, res) => {
  try {
    const result = await runAgent({ db, allowMissingGmail: true, query: req.body?.query, manual: true });
    res.json(result);
  } catch (error) {
